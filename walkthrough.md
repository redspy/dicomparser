...
3.  **이미지 미리보기**:
    ...
    -   **Windowing**: 비압축 이미지의 경우 Window Center/Level 조절 슬라이더를 통해 밝기와 대비를 실시간으로 조절할 수 있습니다.
    -   *참고*: 브라우저 기반 파싱의 한계로 기본 비압축 포맷과 JPEG Baseline 포맷을 지원합니다.
4.  **자동 로드**:
    -   웹 페이지 접속 시, 서버(로컬)에 위치한 `IMG-0002-00001.dcm` 파일을 자동으로 불러와 표시합니다.
5.  **태그 편집 및 저장**:
    -   **값 수정**: 데이터 테이블의 셀을 클릭하여 태그 값을 직접 수정할 수 있습니다.
    -   **태그 추가**: 'Add Tag' 버튼을 통해 새로운 Custom Tag를 추가할 수 있습니다.
    -   **다운로드**: 'Download DICOM' 버튼을 클릭하여 수정된 내용을 새로운 DICOM 파일로 저장할 수 있습니다.
6.  **UI/UX**:
    -   프리미엄 다크 모드 디자인 적용.
    -   반응형 레이아웃.
